...
def get_member_list_sro(id_sro):
    url = f"https://reestr.nopriz.ru/api/sro/{id_sro}/member/list"
    page = 1
    ...
    sro_info_url = f"https://reestr.nopriz.ru/api/sro/{id_sro}"
...
for id in get_sro_id_list():
    for _ in get_member_list_sro(id):
        write_to_file("sro_members_nopriz.txt", ';'.join(_))
